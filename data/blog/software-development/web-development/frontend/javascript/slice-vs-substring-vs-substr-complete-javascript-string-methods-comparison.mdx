...
// Example 1: Negative start index only (same indices as slice() above)
console.log(str.substring(-6));        // "JavaScript"
// Step-by-step:
// 1. startIndex = -6 → treated as 0
...
// Example 2: Both indices negative (same indices as slice() above)
console.log(str.substring(-6, -1));    // "" (empty string)
// Step-by-step:
// 1. startIndex = -6 → treated as 0
// 2. endIndex = -1 → treated as 0
...
// Example 3: Positive start, negative end (same indices as slice() above)
...
// Example 4: Negative start, positive end (same indices as slice() above)
console.log(str.substring(-4, 10));     // "JavaScript"
// Step-by-step:
// 1. startIndex = -4 → treated as 0
// 2. endIndex = 10
// 3. Check: 0 > 10? NO → No swap
// 4. Extract: indices 0-9 → entire string "JavaScript"
...
// Example 5: Both negative, extracting middle (same indices as slice() above)
console.log(str.substring(-8, -3));    // "" (empty string)
// Step-by-step:
// 1. startIndex = -8 → treated as 0
